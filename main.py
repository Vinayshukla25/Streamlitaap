...
gop=['bar','line','area']
c1,c2 =st.columns(2)
sel_op=c1.selectbox("Select the type of plot for popularity",gop) ## selectbox ki jagah radio bhi likh sakte hai
subset=df.sort_values(by='popularity')[:50]

if sel_op == gop[0]:

    # subset=df.sort_values(by='popularity')[:100](hr baar aise bhi likh ke ho sakta hai pr hamne upr ek baar hi likh diya to run ho rha)
...
with t1:
    ...
    # col3=st.radio("Select the third column for 3rd plot",num_cols)
...
with t2:
    ...
    # num_cols =df.select_dtypes(include=np.number).columns.tolist()
    col1=c1.selectbox("Select the first column for 3rd plot",num_cols)
